...
const AvailableSide = ({ isOpen, onClose, refreshKey }) => {
  ...
  const fetchDonations = async () => {
    ...
    try {
      ...
      console.error("AvailableSide fetch error:", err);
  ...
  // run when the sidebar opens OR when refreshKey changes
  useEffect(() => {
    if (isOpen) {
      fetchDonations();
    }
  ...
  return (
    <div className={`available-side ${isOpen ? "visible" : "hidden"}`}>
      ...
      {!loading && !error && donations.length === 0 && (
        <p className="status">No donations available.</p>
      )}
